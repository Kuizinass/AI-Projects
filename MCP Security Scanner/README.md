...
<!-- DEMO GIF HERE: 30s — `blastscope scan` on a messy config → F grade → fix two findings → re-scan → B grade -->
...
| Primary output | Findings list / SARIF | **Blast-radius grade + board-ready risk narrative** (SARIF too) |
...
5. **Honest limitations.** BlastScope assesses capability surface, not server source code (use a SAST tool for that), and an A grade means "no known risk patterns detected", not "safe".
...
- **v0.1** — static config scanning, R1/R4/R7/R9/R10, installation grade, terminal + JSON
